    ...
    public static class Startup
    {
        ...
        public static void Debug()
        {
            foreach (var recipe in DefDatabase<RecipeDef>.AllDefs)
            {
                if (recipe.ProducedThingDef != null)
                {
                    ...
                    if (group != null)
                    {
                        Log.Message("Group for " + recipe.ProducedThingDef + " - " + group.thingKey + " - " + string.Join(", ", group.thingDefs));
                        if (group.MainThingDef != recipe.ProducedThingDef)
                        {
                            Log.Error(recipe + " has a produced thing def not replaced: " + recipe.ProducedThingDef + ", main thing def: " + group.MainThingDef);
                        }
                        else
                        {
        ...
        private static void DebugThingFilter(RecipeDef recipe, ThingFilter thingFilter)
        {
            foreach (var thingDef in (thingFilter.thingDefs ?? new List<ThingDef>()))
            {
                ...
                if (group != null)
                {
                    Log.Message("Group for " + thingDef + " - " + group.thingKey + " - " + string.Join(", ", group.thingDefs));
                    if (group.MainThingDef != thingDef)
                    {
                        Log.Error(recipe + " has an allowed thing def not replaced: " + thingDef + ", main thing def: " + group.MainThingDef);
                    }
                    else
                    {
